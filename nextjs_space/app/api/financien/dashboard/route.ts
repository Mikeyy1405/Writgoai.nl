...
export async function GET(req: NextRequest) {
  try {
    ...
    // Haal alle data parallel op uit Moneybird met expliciete types
    ...
    try {
      const results = await Promise.allSettled([
        withTimeout(moneybird.listSalesInvoices(), API_TIMEOUTS.MONEYBIRD_API, 'Moneybird timeout'),
        withTimeout(moneybird.listSubscriptions(), API_TIMEOUTS.MONEYBIRD_API, 'Moneybird timeout'),
        withTimeout(moneybird.getPurchaseInvoices(), API_TIMEOUTS.MONEYBIRD_API, 'Moneybird timeout'),
        withTimeout(moneybird.listContacts(), API_TIMEOUTS.MONEYBIRD_API, 'Moneybird timeout'),
      ]);

      if (results[0].status === 'fulfilled') salesInvoices = results[0].value as any[];
      if (results[1].status === 'fulfilled') subscriptions = results[1].value as any[];
      if (results[2].status === 'fulfilled') purchaseInvoices = results[2].value as any[];
      if (results[3].status === 'fulfilled') contacts = results[3].value as any[];
    } catch (error) {
      console.error('[Financien Dashboard] Error fetching Moneybird data:', error);
    }

    // Bereken KPIs uit Moneybird data
    const paidInvoices = salesInvoices.filter((inv) => inv.state === 'paid');
    const openInvoices = salesInvoices.filter((inv) => 
      inv.state === 'open' || inv.state === 'pending_payment'
    );
    const lateInvoices = salesInvoices.filter((inv) => inv.state === 'late');
    const draftInvoices = salesInvoices.filter((inv) => inv.state === 'draft');
    ...
    const lateAmount = lateInvoices.reduce((sum, inv) => 
      sum + parseFloat(inv.total_unpaid || '0'), 0
    );
    ...
    const monthlyExpenseInvoices = purchaseInvoices.filter((inv) => {
      if (!inv.invoice_date) return false;
      const invoiceDate = new Date(inv.invoice_date);
      return invoiceDate >= startOfMonth && inv.state === 'paid';
    });

    const expenses = monthlyExpenseInvoices.reduce((sum, inv) => 
      sum + parseFloat(inv.total_price_incl_tax || '0'), 0
    );

    // Bereken inkomsten deze maand
    const monthlyInvoices = salesInvoices.filter((inv) => {
      if (!inv.invoice_date) return false;
      const invoiceDate = new Date(inv.invoice_date);
      return invoiceDate >= startOfMonth && inv.state === 'paid';
    });
