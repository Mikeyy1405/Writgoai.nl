...
const IMAGE_MODEL_OPTIONS = [
  ...
  },
  {
    id: 'GPT_IMAGE_1',
    name: 'GPT Image 1',
    description: 'Duurste optie met stijlopties (weinig gebruikt)',
    quality: 'Premium+',
    speed: '⚡⚡ Snel',
    credits: 18,
    recommended: false,
...
// GPT Image 1 Stijlopties (niet exporteren vanwege Next.js restrictie)
const GPT_IMAGE_STYLES = [
  { id: 'natural', name: 'Natuurlijk', description: 'Natuurlijke, evenwichtige stijl' },
  { id: 'vivid', name: 'Levendig', description: 'Heldere kleuren en dynamische compositie' },
  { id: 'realistic', name: 'Realistisch', description: 'Fotorealistische afbeeldingen' },
  { id: 'cinematic', name: 'Filmisch', description: 'Cinematografische belichting en compositie' },
  { id: 'anime', name: 'Anime', description: 'Japanse anime stijl' },
  { id: 'comic', name: 'Comic', description: 'Comic book stijl' },
  { id: 'watercolor', name: 'Aquarel', description: 'Aquarel schilderij stijl' },
  { id: 'oil-painting', name: 'Olieverf', description: 'Klassieke olieverf stijl' },
  { id: 'sketch', name: 'Schets', description: 'Potlood schets stijl' },
  { id: '3d-render', name: '3D Render', description: 'Hoogwaardige 3D gerenderde afbeelding' },
  { id: 'minimalist', name: 'Minimalistisch', description: 'Simpel en clean design' },
  { id: 'abstract', name: 'Abstract', description: 'Abstracte kunst stijl' },
  { id: 'fantasy', name: 'Fantasy', description: 'Fantasierijke, magische stijl' },
  { id: 'sci-fi', name: 'Sci-Fi', description: 'Science fiction stijl' },
  { id: 'vintage', name: 'Vintage', description: 'Retro, vintage look' },
  { id: 'professional-photo', name: 'Professionele Foto', description: 'Studio fotografie kwaliteit' },
] as const;
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Return available models, styles, and their pricing
    return NextResponse.json({
      success: true,
      models: IMAGE_MODEL_OPTIONS,
      styles: GPT_IMAGE_STYLES,
    });
  } catch (error: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { 
      prompt, 
      model = 'NANO_BANANA',  // Ultra cost-optimized default: alleen 2 credits!
      style = 'natural',
      quality = 'medium',
      width,
      height
    ...
    console.log(`🎨 Generating image with ${modelConfig.name} (${modelConfig.credits} credits)`, {
      style: model === 'GPT_IMAGE_1' ? style : 'N/A',
      quality,
    });
...
Important: Create a unique, contextually relevant image that matches the specific content described above. Avoid generic stock imagery. Focus on visual storytelling that enhances the written content.`;
    ...
    // Map quality from frontend to API format (for backward compatibility)
    const apiQuality: 'low' | 'medium' | 'high' = 
    ...
    const result = await generateImage({
      ...
      num_images: 1,
      style: model === 'GPT_IMAGE_1' ? style : undefined,
      quality: apiQuality,
      width,
      height,
    });
    ...
    await deductCredits(
      client.id,
      modelConfig.credits,
      `AI Afbeelding: ${modelConfig.name}${model === 'GPT_IMAGE_1' ? ` (${style})` : ''}`,
