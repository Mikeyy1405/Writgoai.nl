...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Er ging iets mis bij het ophalen van content analyse' },
      { status: 500 }
    );
  }
}

// POST - Sla content analysis op
export async function POST(
  ...
  try {
    ...
    const { contentAnalysis, status } = body;

    // Validatie
    if (!contentAnalysis) {
      return NextResponse.json(
        { error: 'Content analysis data is verplicht' },
        { status: 400 }
      );
    }
    ...
    const project = await prisma.project.updateMany({
      ...
      data: {
        contentAnalysis,
        contentAnalysisStatus: status || 'completed',
    ...
    return NextResponse.json({
      success: true,
      message: 'Content analyse opgeslagen',
      project: updatedProject,
    });
  } catch (error: any) {
    console.error('Error saving content analysis:', error);
    return NextResponse.json(
      { error: 'Er ging iets mis bij het opslaan van content analyse' },
      { status: 500 }
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error('Error updating content analysis status:', error);
    return NextResponse.json(
