...
export default function TopicalAuthorityListPage() {
  ...
  const generateArticle = async (articleId: string) => {
    try {
      ...
      const prepResponse = await fetch('/api/client/topical-authority/generate-article', {
        method: 'POST',
      ...
      if (!prepData.success) {
        alert('Fout bij voorbereiden: ' + (prepData.details || prepData.error));
        setGeneratingId(null);
        return;
      }

      const articleData = prepData.data;
      
      // Step 2: Generate content using the unified content generator
      console.log('[Topical Authority] Starting content generation for:', articleData.title);
      
      const generateResponse = await fetch('/api/client/content/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          topic: articleData.title,
          language: 'NL', // TODO: Get from map settings
          wordCount: articleData.wordCountTarget || 1500,
          imageCount: 3,
          projectId: map?.projectId || undefined,
          includeImages: true,
          includeFAQ: true,
          includeYouTube: false,
          tone: 'professional',
          keywords: articleData.keywords || articleData.focusKeyword,
          topicalContext: {
            pillarTitle: articleData.pillarTitle,
            subtopicTitle: articleData.subtopicTitle,
            niche: articleData.niche,
            searchIntent: articleData.searchIntent,
            internalLinks: articleData.internalLinks,
            dataForSEO: articleData.dataForSEO,
          },
        }),
      });

      if (!generateResponse.ok) {
        throw new Error(`Content generatie gefaald: ${generateResponse.status}`);
      }
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.contentId) {
                  generatedContentId = data.contentId;
                }
                if (data.done && data.success) {
                  console.log('[Topical Authority] Content generated successfully:', generatedContentId);
                }
              } catch (e) {
                // Skip invalid JSON
              }
      ...
      // Step 3: Update article status to 'generated'
      if (generatedContentId) {
        await fetch(`/api/client/topical-authority/articles/${articleId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
            status: 'generated',
        ...
        alert('✅ Artikel gegenereerd! Bekijk het in de Content bibliotheek.');
        
        // Refresh the list
        await loadArticles();
      } else {
        alert('⚠️ Artikel gegenereerd maar contentId niet ontvangen');
      }
      
    } catch (error: any) {
      console.error('[Topical Authority] Generation error:', error);
