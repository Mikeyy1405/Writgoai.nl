...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!password) {
      return NextResponse.json(
        { error: 'WordPress password ontbreekt' },
        { status: 400 }
      );
    }
    ...
    console.log(`Fetching WordPress posts (page ${page}, per_page ${perPage})`);
    ...
    return NextResponse.json(
      { error: error.message || 'Fout bij ophalen van WordPress posts' },
      { status: 500 }
    );
  }
}
