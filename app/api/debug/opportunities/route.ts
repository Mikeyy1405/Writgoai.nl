...
export async function GET() {
  try {
    ...
    // Group by status
    const byStatus = allOpps?.reduce((acc: any, opp) => {
    ...
    const recent = allOpps?.filter(o => o.created_at > oneDayAgo);
    
    // Get detected ones
    const detected = allOpps?.filter(o => o.status === 'detected');
    ...
    return NextResponse.json({
      ...
      recentOpportunities: recent?.slice(0, 10).map(o => ({
        id: o.id,
        title: o.title,
        status: o.status,
        priority: o.priority,
      ...
      detectedOpportunities: detected?.slice(0, 10).map(o => ({
        id: o.id,
        title: o.title,
        priority: o.priority,
        created_at: o.created_at
