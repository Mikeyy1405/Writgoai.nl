...
export default function LibraryPage() {
  ...
  async function publishArticle(article: Article) {
    ...
    try {
      ...
      alert(`Fout: ${err.message || 'Fout bij publiceren'}`);
    } finally {
      setPublishing(false);
      setSelectedArticle(null);
    }
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-white mb-4">Artikel verwijderen?</h3>
            <p className="text-gray-400 mb-6">
              Weet je zeker dat je dit artikel wilt verwijderen? Dit kan niet ongedaan worden gemaakt.
            </p>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-gray-400 text-sm line-clamp-2">
                      {article.content.replace(/<[^>]*>/g, '').replace(/```html/g, '').substring(0, 200)}...
                    </p>
                  </div>
                </div>
