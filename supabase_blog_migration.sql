-- WritGo.nl Blog & AutoPilot System Migration
-- This creates tables for the blog system and AutoPilot for WritGo.nl itself
...
CREATE TABLE IF NOT EXISTS article_tag_mapping (
  article_id UUID REFERENCES articles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS writgo_autopilot_config (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS writgo_keywords (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS writgo_gsc_data (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  date DATE NOT NULL,
  page TEXT NOT NULL,
  query TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS writgo_performance_insights (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  article_id UUID REFERENCES articles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS writgo_activity_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS writgo_content_queue (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_writgo_gsc_data_date ON writgo_gsc_data(date DESC);
CREATE INDEX IF NOT EXISTS idx_writgo_gsc_data_page ON writgo_gsc_data(page);
CREATE INDEX IF NOT EXISTS idx_writgo_keywords_status ON writgo_keywords(status);
CREATE INDEX IF NOT EXISTS idx_writgo_performance_insights_status ON writgo_performance_insights(status, priority);
CREATE INDEX IF NOT EXISTS idx_writgo_content_queue_status ON writgo_content_queue(status, priority DESC);
